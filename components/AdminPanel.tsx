...
interface AdminPanelProps {
  ...
  initialGifts: Gift[];
  onSave: (people: Person[], gifts: Gift[]) => void;
  onClose: () => void;
}
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ initialPeople, initialGifts, onSave, onClose }) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, id: number, type: 'person' | 'gift') => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        if (type === 'person') {
          handlePersonChange(id, 'photoUrl', result);
        } else {
          handleGiftChange(id, 'photoUrl', result);
        }
      };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `christmas-exchange-config-${new Date().toISOString().slice(0, 10)}.json`;
      ...
      reader.onload = (event) => {
        try {
          ...
          if (data.people && data.gifts) {
             // Validate length to match UI expectations if needed, but for now just load it
             // Reset drawing status on import to ensure clean slate
  ...
  const handleSave = () => {
    if (window.confirm("Save changes and restart the game? (Progress will be reset)")) {
        onSave(people, gifts);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-[100] flex flex-col p-4 md:p-8 overflow-hidden font-sans text-white">
      <div className="flex justify-between items-center mb-6 border-b border-white/20 pb-4">
        <h2 className="text-3xl font-bold text-christmas-gold">⚙️ Admin Dashboard</h2>
        ...
        <div className="grid gap-4">
          ...
          {activeTab === 'gifts' && gifts.map((gift) => (
             <div key={gift.id} className="bg-gray-900 p-4 rounded flex flex-col md:flex-row gap-4 items-center border border-gray-700">
               ...
               <div className="flex-grow w-full md:w-auto">
                  <label className="text-xs text-gray-400 block mb-1">Description (for AI & Reveal)</label>
                  <input 
                     type="text" 
      ...
      <div className="mt-6 flex justify-end gap-4 border-t border-white/20 pt-4">
        <span className="text-yellow-500 text-sm self-center mr-auto">
           ⚠️ Note: Large images may slow down the app. Try to use small JPGs/WebP under 500KB.
        </span>
        <button 
           onClick={onClose}
           className="px-6 py-3 rounded font-bold text-gray-300 hover:bg-gray-800 transition-colors"
        >
           Cancel
        </button>
        <button 
           onClick={handleSave}
           className="bg-christmas-red text-white px-8 py-3 rounded font-bold hover:scale-105 transition-transform shadow-lg border border-christmas-gold"
        >
